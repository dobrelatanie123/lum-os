...
import type { PaperResult, MatchScore, ScoredPaper } from './verification-types.js';
...
export class MatchScorer {
   ...
   */
  score(paper: PaperResult, claim: SynthesizedClaim): ScoredPaper {
   ...
   */
  rankPapers(papers: PaperResult[], claim: SynthesizedClaim): ScoredPaper[] {
    return papers
  ...
  private scoreAuthorMatch(paper: PaperResult, claim: SynthesizedClaim): number {
    ...
    for (const paperAuthor of paper.authors) {
      ...
      if (paperAuthorLower.includes(claimAuthor)) return 1.0;
      
      // Surname match
      if (paperSurname === claimSurname) return 0.9;
  ...
  private scoreTopicMatch(paper: PaperResult, claim: SynthesizedClaim): number {
    // Get keywords from claim query and finding
    const claimKeywords = this.extractKeywords(
      claim.search.primary_query + ' ' + claim.extraction.finding_summary
    );
    ...
    const matches = claimKeywords.filter(k => 
      paperKeywords.some(pk => pk.includes(k) || k.includes(pk))
    );
  ...
  private scoreYearPlausibility(paper: PaperResult): number {
    if (!paper.year) return 0.5; // Unknown year = neutral
  ...
  private scoreAbstractRelevance(paper: PaperResult, claim: SynthesizedClaim): number {
    if (!paper.abstract) return 0.3; // No abstract = low confidence
    ...
    if (findingWords.length === 0) return 0.3;
  ...
  private getMatchQuality(
    ...
    if (totalScore >= 0.7) return 'strong';
    if (totalScore >= 0.5) return 'moderate';
