...
export const KNOWN_RESEARCHERS = {
    'candow': {
 ...
 */
export function normalizeAuthor(mentioned) {
    if (!mentioned) {
 ...
 */
export function generatePhoneticVariants(name) {
    const variants = new Set([name]);
    // K/C swap at start
    if (name.startsWith('k'))
        variants.add('c' + name.slice(1));
    if (name.startsWith('c'))
        variants.add('k' + name.slice(1));
    // Double letter variations
    variants.add(name.replace(/(.)\1/g, '$1'));
    // -er/-or endings
    if (name.endsWith('er'))
        variants.add(name.slice(0, -2) + 'or');
    if (name.endsWith('or'))
        variants.add(name.slice(0, -2) + 'er');
    // -man/-mann endings
    if (name.endsWith('mann'))
        variants.add(name.slice(0, -1));
    if (name.endsWith('man'))
        variants.add(name + 'n');
    // -son/-sen endings
    if (name.endsWith('son'))
        variants.add(name.slice(0, -2) + 'en');
    if (name.endsWith('sen'))
        variants.add(name.slice(0, -2) + 'on');
    return [...variants];
}
